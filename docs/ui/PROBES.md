...
**Eleven** CDP drivers in `scripts/`, two diff tools (`css_type_diff.mjs`,
`css_margin_diff.mjs`), plus one plain parser (`css_breakpoints.mjs`, UI-064 — it
reads CSS text, drives no browser, and is listed below because both probes now import it).

⚠ **The count said "Eight" until 2026-09-08 and was wrong by two before this task
touched it.** `css_shot.mjs` (UI-101) had been in `scripts/` since 2026-09-08 and was
never listed here at all; `auth_lock_probe.mjs` (UI-108) is the eleventh. Both rows are
below now. Enumerate with `ls scripts/*.mjs` rather than trusting this sentence — the
same rule this epic applies to every other number. They are the reason the numbers on `STATUS.md` can be
...
| `css_shot.mjs` | screen | **Not a measurement — a PNG.** No pass/fail, no ratchet. It exists because this repo had seven ways to measure a page and **none to look at one**, a gap D41/D62/D26 each hit in turn, and which turned expensive on 2026-09-08: eight pages were moved onto the Panze shell, every number came back green — computed styles, contrast, print, tests — and Irfan said *"mujhe kuch nazar nahi aa raha"*. Ten pages, 1280x900 viewport by default; `--full` captures the whole page (`bank` is ~43,000px tall, which is why it is not the default). Written for UI-101; it then caught three layout breakages in one batch that no metric could see. |
| `auth_lock_probe.mjs` | screen | **Does the idle auto-lock actually lock, and does the key gate tell the truth?** The only probe here that is not about CSS, and it exists for the reason `tests/test_auth.py` states in its own docstring: *"ek boundary jise koi gate nahi dekhta, wo agle refactor mein khamoshi se khul sakti hai."* Auto-lock (UI-108) is entirely client-side and this repo has **no JS test harness — not even a `package.json`** — so pytest cannot see it at all. **Seven cases across TWO PHASES, and the split is the point.** *Phase 1, auth OFF (4 cases):* fresh key stays open and mounts the Lock button, a 31-minute-old key is wiped **and the gate opens on that same load**, no key (dev) shows neither gate nor button, clicking Lock clears immediately. Auth must be **off** here or the gate's cause is unattributable — a 401 gate and a lock gate look identical. *Phase 2, auth ON with a known test key (3 cases):* what the gate **says** — no key stored must not accuse the user, a genuinely wrong key must say so plainly, and a correct key must get in (the third proves the first two are a message difference and not broken auth). **⚠ Phase 2 exists because phase 1's blind spot shipped a real bug**: UI-108 was 4/4 green while every 401 said *"Key ghalat ya missing hai"*, so a first-time visitor who had entered nothing was told their key was wrong — Irfan hit it on the first try (UI-109). A probe that cannot enter a state cannot defend it. **It starts both uvicorns itself** with `PAPER_MAKER_API_KEY` set explicitly — empty for phase 1, the test key for phase 2. Empty, not unset: an empty value still counts as present in `os.environ`, so `load_dotenv(override=False)` will not fill it, and the real `.env` key never reaches either phase (UI-107's lesson — the day a key landed in `.env`, all 1,106 tests fell over on `assert 401 == 200`). `--shot <dir>` writes a PNG of the mounted button, because `querySelector` proves a button **exists**, not that it **looks right**. |
