...
| `auth_lock_probe.mjs` | screen | **Does the idle auto-lock actually lock?** The only probe here that is not about CSS, and it exists for the reason `tests/test_auth.py` states in its own docstring: *"ek boundary jise koi gate nahi dekhta, wo agle refactor mein khamoshi se khul sakti hai."* Auto-lock (UI-108) is entirely client-side and this repo has **no JS test harness — not even a `package.json`** — so pytest cannot see it at all. Four cases on headless Edge: fresh key stays open and mounts the Lock button, a 31-minute-old key is wiped **and the gate opens on that same load**, no key (dev) shows neither gate nor button, and clicking Lock clears immediately. **⚠ It starts its own uvicorn with `PAPER_MAKER_API_KEY=""`**, for two reasons: with auth ON every load 401s into the gate and the gate's cause becomes unattributable; and UI-107's lesson — the day a key landed in `.env`, all 1,106 tests fell over on `assert 401 == 200` because the suite read a developer's `.env`. Empty, not unset: an empty value still counts as present in `os.environ`, so `load_dotenv(override=False)` will not fill it. `--shot <dir>` writes a PNG of the mounted button, because `querySelector` proves a button **exists**, not that it **looks right**. |
