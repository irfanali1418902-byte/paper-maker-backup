...
migrated; **`blueprint` HELD on Irfan's call**) — Sprint 3 2/3; **three pages are live on the
new tree**, and **three are now HELD**: `landing`, `taqseem` and `blueprint`. The three that
remain migratable are `bank`, `index` and `print`.

---

## NEXT TASK → **UI-032** — migrate `bank`, `index`, `print`. **`blueprint` is HELD.**
...
**What UI-032 now means: `bank`, `index`, `print`, one at a time, with a browser open.** Both
checks have been run on all three and all three are repetitions of `slo-health`/`library` —
rule exposure **0 / 3 / 0**, token exposure **0 / 0 / 0**. `blueprint` is out of scope for the
migration; it is measured, held, and waiting with `landing` and `taqseem`. **Nothing has been
parked for it — no entry file was written, per the rule that the decision comes first.**
