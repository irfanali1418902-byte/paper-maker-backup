...
- **Auth (SEC-02, 2026-09-12):** asli users — per-teacher account, scrypt password, server-side session (HttpOnly cookie, 30 min idle + 12 ghante sakht hadd), roles (admin/teacher), lockout (5 → 15 min, naam par), aur `auth_events` ka log. Mode DATA se badalta hai: koi user na ho to app haraf ba haraf purani (shared-key) tarah chalti hai. Tafseel `docs/AUTH.md`.
...
| ~~4~~ ✅ | ~~**Lesson Plan module** — plans linked to topics + coverage report~~ | **DONE 2026-08-22/23 — AND IT SHIPPED UNDER ANOTHER NAME, WHICH IS WHY THIS ROW STAYED OPEN FOR DAYS AFTER IT WAS FINISHED.** It is `R7 — Hafta-war Plan + Coverage`, spec `docs/TOPIC_WEEK_PLAN.md`, and its §1 records the naming decision: "Lesson Plan" was rejected because the module does not hold lesson content, it maps **topics to weeks**. All four marhale are done — table + repository + service + API (50 tests), Excel import + template (29), coverage service + `GET /api/topic-plan/coverage` (20), and `plan.html` with the week-strip coverage badges. **It was not the "new module, size L" this row assumed**: §2 measured that half the machinery already existed, and the whole thing landed in four days. <br><br>~~⚠ **Built ≠ adopted.** `topic_week_plan` holds **4 rows** (measured 2026-08-25) … The remaining work is not code — it is one teacher filling in one Excel plan.~~ <br><br>**2026-08-26 — ADOPTED FOR PRE YEAR 3, aur us ki Excel teen din se yahin pari thi.** `namoona_plan_pre_year_3.xlsx` root mein untracked para tha, **2026-08-23 se, poora bhara hua** — 87 rows, weeks 1–36, koi khali `week_no` nahi, saare topic id PY3 se match. Yani "baqi kaam" pehle se ho chuka tha aur sirf **import nahi hua tha**. `POST /api/topic-plan/assign-import` → `updated: 87, errors: []`. `topic_week_plan` **4 → 91**. <br><br>⚠ **`covered: 0` par na chaunkein:** `topic_coverage_service.py`:54 covered ko **`papers` se** ginta hai, bank se nahi. PY3 ke 43 topics par 172 sawal hain magar koi paper nahi bana, to 0 durust hai. PY1 wohi **49/81, 60%** deta hai. <br><br>**Baqi:** PY2 aur PY1 ke plan ab bhi khali hain — wohi "ek Excel" wala kaam, magar ab uska namoona mojood hai. | L |
